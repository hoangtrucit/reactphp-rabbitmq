...
$client->connect()->then(function (Client $client) {
    ...
    //init exchange
    return $channel
        ->exchangeDeclare('events', 'topic')->then(
    ...
    // init queue
    return $channel
        ->queueDeclare('test', false, true, false, false)->then(
        function () use ($channel) {
            return $channel;
        });
})->then(function (Channel $channel) {
    // binding queue to exchange and specify routingKey
    return $channel
        ->queueBind('test','events','user.create')->then(
        function () use ($channel) {
            return $channel;
        });
